...
`benchmark_manifest.py` is the single source of truth for benchmark categories, dependency metadata, and quick-mode overrides. `run_all_tests.py` imports that manifest, applies quick-mode settings, runs top-level `perf_test*` functions in source order, and can emit text or JSON results.
